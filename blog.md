...
## 動的に文字を変えてみる
文字は表示できしたが、これだとインタラクション性が皆無なので面白くないです。

そこで、先程追加したテキストエリアに自分が見ているものを表示できるように修正しましょう！

ここで行うのは

* 自分が見てる場所がわかるようにカーソルを追加する
* オブジェクトを一定時間見つめるとイベントが発火されるようなscript(リスナー)を実装する
* イベント発生時にオブジェクトのidを取得して、テキストエリアに代入するscriptを実装する
* オブジェクトに対して上記リスナーを追加する(ついでにidも)
...
まずは`<a-camera>`の下に`<a-cursor>`を追加し、自分の目線の位置を確認できるようにします。

次に`AFRAME.registerComponent`で`cursor-listener`コンポーネントを定義し、`<a-box>`などのオブジェクトにコンポーネントとして追加しました。

これによって、オブジェクトにクリックイベントを受け取れるようになります。
