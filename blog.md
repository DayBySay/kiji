...
この記事を読んだ方に、**VRではどんなコンテンツが作れるのか？**と、**どんな開発環境があるのか？**、**どんな事に気をつければよいのか？**ということを知って頂き、今後のVRコンテンツ開発に役立てて頂ければ幸いです！
...
どうしてもトラッキングを止めなければ行けないときは画面をフェードアウトさせること、またその時にオーディオを流してアプリが起動中であることをユーザに示しましょう。フェードアウトにかける時間が早すぎるとユーザが混乱してしまうので、十分な時間をかけてフェードアウトさせるのが有効です。
...
など、色々やりかたはあります。大きい方向性としては、ユーザの記憶や経験と違う表現や、期待・予測しているものと違う表現をしないということのようです。
...
レティクルを表示する場合、立体的に、奥行きを感じられるようにターゲットのオブジェクト上に投影しましょう。レティクルとターゲットの間を開けすぎると、両方を同時に捉えるのが難しくなってしまい違和感を感じます。レティクルのサイズは奥行きに比例させるか、常に固定のサイズを維持することでみやすさを担保しましょう。
...
### ヒューズボタンを使う
ヒューズボタンとは、ユーザが特定の時間見つめたときにトリガーされるボタンのことです。

上記の説明の通り、ヒューズボタンはオブジェクトを数秒間見つめる必要があります。これはアクションが実行されるまで待つ必要があり、遅く感じる上にユーザにストレスを与えることを意味しています。
...
視線ベースのインタラクションは多くのユーザにとって慣れないインタフェイスですので、なるべく疲れないよう気遣いをすることが重要です。
