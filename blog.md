...
また、モバイルHMDの中でもハイエンドとローエンドで別れています。
...
現在開発が盛んに行われているのは、Oculus Rift、HTC Vive、Playstation VRの3つのHMDですので、これらをとりあげました。
...
* 既存のストアを利用してアプリを配布できる

悪い点
* 現状のモバイルHMDだとポジショントラッキングができないので、コンテンツ内での移動に制約がある
* 多くの場合ハンドコントローラがないので、コンテンツ内のインタフェイスが視線ベースになることが多く、疲れやすい
* スマホのスペックによって扱えるポリゴン数にかなり差があるので調整が大変
